...
function addExpander() {
    $('.primary-nav__item').each(function () {
        if ($(this).children('ul').length > 0) {
            $(this).append('<div class=\"secondary-expander\"><img src=\"/PbcLinear/img/svg/circle-arrow-grey.svg\" class=\"nav-expander__image\" alt=\"Navigation Arrow\" /></div>');
            //$(this).append('<div class=\"secondary-expander\"></div>');
        }
    })
    $('.secondary-nav__item').each(function () {
        if ($(this).children('ul').length > 0) {
            $(this).append('<div class=\"tertiary-expander\"><img src=\"/PbcLinear/img/svg/circle-arrow-blue.svg\" class=\"nav-expander__image\" alt=\"Navigation Arrow\" /></div>');
        }
    })
}
...
function expanderNav() {
    $('.secondary-expander').unbind('click').on('click', (function (e) {
        if ($(this).hasClass('expander-open')) {
            $(this).removeClass('expander-open').siblings('.secondary-nav').css({ height: '0' }).children().find('.tertiary-expander').removeClass('tertiary-expander-open').siblings('.tertiary-nav').css({ height: '0' });
        }
        else {
            $('.secondary-expander').removeClass('expander-open').siblings('.secondary-nav').css({ height: '0' }).children().find('.tertiary-expander').removeClass('tertiary-expander-open').siblings('.tertiary-nav').css({ height: '0' });
            e.preventDefault();
            e.stopPropagation();
...
function tertiaryExpander() {
    $('.tertiary-expander').unbind('click').on('click', (function (e) {
        if ($(this).hasClass('tertiary-expander-open')) {
            $(this).removeAttr('style').removeClass('tertiary-expander-open').siblings('ul').stop(true, true).animate({ height: '0' }, 100);
        }
        else {
            $('.tertiary-expander').removeAttr('style').removeClass('tertiary-expander-open').siblings('.tertiary-nav').removeAttr("style");
            e.preventDefault();
            e.stopPropagation();
            $(this).addClass('tertiary-expander-open').siblings('.tertiary-nav').css({ height: "auto" });
...
function desktopNav() {
    $('.secondary-expander').removeClass('expander-open').siblings('.secondary-nav').css({ height: '0' }).children().find('.tertiary-expander').removeAttr('style').removeClass('tertiary-expander-open').siblings('.tertiary-nav').css({ height: '0' });
...
function tertiaryNav() {
    ...
    $('.secondary-nav__item').hover(function (e) {
        ...
        e.preventDefault();
        e.stopPropagation();
        $(this).find('.tertiary-nav').stop(true, true).delay(100).animate({ height: subHeight }, 250).siblings('.nav-expander').addClass('tertiary-expander-open');
    },
    function (e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).find('.tertiary-nav').stop(true, true).animate({ height: '0' }, 100).siblings('.nav-expander').removeClass('tertiary-expander-open');
    });
}
...
function mobileNav() {
    ...
    $('.nav-launch').removeClass('nav-open').unbind('click').on('click', (function (e) {
        ...
        var childrenHeight = $('.nav-consultant, .main-nav, .utility-nav').children().each(function () {
            var thisHeight = $(this).height();
            mobileNavHeight = thisHeight += mobileNavHeight - 5;
        });
