...
public partial class PbcLinear_Web_PbcLinear_Modules_createProductAddToCart : CMSUserControl
{
    #region "Variables"
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (SKUID == 0)
        {
            ...
            var skuName = "ConfiguredProduct" + newProduct.SKUID;
            newProduct.SKUName = skuName;
            ...
            SKUInfo updateProduct = SKUInfoProvider.GetSKUs()
                                            .WhereEquals("SKUName", skuName)
                                            .LastOrDefault();
            if (updateProduct != null)
            {
                // Updates the product properties
                updateProduct.SKUName = "ConfiguredProduct" +
                                        "" + updateProduct.SKUID;
        ...
        if (SKUID > 0)
        {
            ...
            if (AlwaysShowTotalPrice || (ShowTotalPrice && SKUHasOptions))
            {
                ...
                else
                {
                    // Use default label
                    lblPrice.Text =
                        GetString(ShowPriceIncludingTax
                            ? "ShoppingCartItemSelector.TotalPriceIncludeTax"
                            : "ShoppingCartItemSelector.TotalPriceWithoutTax");
                }
